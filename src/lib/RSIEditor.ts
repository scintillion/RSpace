...
export class RSIEditor {
	...
	public Populate(): void {
        ...
        const FirstVID: RS1.vID = this.RSI.toSortedVIDs[0] as RS1.vID;

		// const vIDs = this.qList.toSortedVIDs;
        const vIDs = this.RSI.toSortedVIDs;
	...
	private CLToSelect(reload?: boolean): void {
        ...
        this.RSI.toSelect(this.selectbox);
	...
	private CreateVID(): void {
        ...
        let vID: RS1.vID = new RS1.vID(this.i.name.value, this.RSI);
		vID.Desc = validDesc;
	...
	private UpdateVID(name: string): void {
        ...
        let vID = new RS1.vID(`${name}:${validDesc}`, this.RSI);
